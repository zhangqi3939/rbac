...
//baidu map
	...
	function addMark_baidu(){
		var points = JSON.parse(window.localStorage.getItem('new_searchData'));
...
function makeInfoWindow(box,address){
	...
	info += '<tr><td class="b">温度1：</td><td>'+box.gps_humi+'°C</td>';
	info += '<td class="b">温度2：</td><td>'+box.gps_temp1+'°C</td></<td>';
	info += '<tr><td class="b">温度3：</td><td>'+box.gps_temp2+'°C</td>';
	info += '<td class="b">温度4：</td><td>'+box.gps_temp3+'°C</td></<td>';
	info += '<tr><td class="b">温度5：</td><td>'+box.cooler_rpm+'°C</td></<td>';
	info += '<td class="b">电量：</td><td>'+box.reserve5+'%</td></<td>';
	info += '<tr><td class="b">电瓶电压：</td><td>'+box.reserve6+'V</td>';
	info += '<td class="b">采样频率：</td><td>'+box.reserve6+'分</td></<td>';
	info += '<tr><td class="b">X轴：</td><td>'+box.reserve7+'mg</td>';
	info += '<td class="b">Y轴：</td><td>'+box.cooler_serial_num+'mg</td></<td>';
	info += '<tr><td class="b">Z轴：</td><td colspan="3">'+box.reserve8+'mg</td></tr>';
...
//full map 页面
	...
	function loadTrack(){
		...
		//加载轨迹
				$.ajax({
					url:  reqDomain + '/D/map_container_track_json',
					type: 'POST',
					data: {
						id: selectedBoxID,
	...
	var overlaycomplete = function(e) {
		...
		if(e.overlay.radius) {
			if(e.overlay.radius>10000){
				layer.msg('半径不能大于10公里！',{'zIndex':19891017,'time':1500});
				$("#fence_radius").val(10000);
			}else{
				$("#fence_radius").val(e.overlay.radius);
			}
		} else {
			if(e.overlay.Ca>10000){
				layer.msg('半径不能大于10公里！',{'zIndex':19891017,'time':1500});
				$("#fence_radius").val(10000);
			}else{
	...
	//保存围栏
		function saveFence() {
			...
			var point = new BMap.Point(fence_longitude,fence_latitude);
			var myIcon = new BMap.Icon("../img/baojing4.png", new BMap.Size(41,51));
			var markerIcon = new BMap.Marker(point,{icon:myIcon});
