...
$("#indexTable").bootstrapTable({
	...
	rowStyle:function(row){
		...
		}else if(row.insert_time == '-' || new Date().getTime() - new Date(row.insert_time).getTime() > 24*60*60*1000){
			//休眠
...
$('.stateBtn .onLine').click(function(){
	...
	var data = [];
	for(var i=0;i<tableData.length;i++){
		if(tableData[i].cooler_alarm_cnt!=0 && tableData[i].cooler_alarm_cnt != null || tableData[i].insert_time == '-' || new Date().getTime() - new Date(tableData[i].insert_time).getTime() > 24*60*60*1000){
			continue
		}else{
			data.push(tableData[i]);
		}
...
$('.stateBtn .outLine').click(function(){
	...
	var data = [];
	for(var i=0;i<tableData.length;i++){
		if((tableData[i].insert_time == '-' || new Date().getTime() - new Date(tableData[i].insert_time).getTime() > 24*60*60*1000) && tableData[i].cooler_alarm_cnt == 0){
			data.push(tableData[i]);
		}
	}
	$('#indexTable').bootstrapTable('load',data);
