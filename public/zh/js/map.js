...
//full map 页面
	...
	function loadTrack(){
		...
		//加载轨迹
				$.ajax({
					url: reqDomain +'/device/map_container_track_json',
					type: 'POST',
					data: {
						id: selectedBoxID,
					...
					success: function(json) {
						...
						if(json.code == '200') {
							...
							for(var i = fenceList.length - 1; i >= 0; i--) {
								var point = new BMap.Point(fenceList[i].lng_bd, fenceList[i].lat_bd);
								var fenceName = fenceList[i].name;
								var label = new BMap.Label(fenceName, {
									position: point,offset:new BMap.Size(24,4)
								});
								label.setStyle({border:"0",fontSize:"16px",fontFamily:"微软雅黑",maxWidth:"none",backgroundColor:"0.000000000001",});
								var marker = new BMap.Marker(point);
								marker.setIcon(new BMap.Icon('/img/station.png',new BMap.Size(24,24)));
								marker.setLabel(label);
								marker.addEventListener('click',function(e){
									console.log(fenceList[i].name);
									//openInfo_baidu(points[i],e);
								});
								map.addOverlay(marker);
								// markers.push(marker);
