...
abstract class Builder
{
    ...
    protected function parseData($data, $options)
    {
        ...
        foreach ($data as $key => $val) {
            ...
            if (false === strpos(trim($key,'"'), '.') && !in_array(trim($key,'"'), $fields, true)) {
                if ($options['strict']) {
                ...
                if (0 === strpos($val, ':') && $this->query->isBind(substr($val, 1))) {
                    ...
                    $this->query->bind('data__' . trim($key,'"'), $val, isset($bind[trim($key,'"')]) ? $bind[trim($key,'"')] : PDO::PARAM_STR);
                    $result[$item] = ':data__' . trim($key,'"');
                }
            }
        }
        return $result;
    ...
    public function insert(array $data, $options = [], $replace = false)
    {
        ...
        //$options['sequence']=$options['table'].'_seq';
