...
class Query
{
    ...
    protected function getFieldBindType($type)
    {
        if (0 === strpos($type, 'set') || 0 === strpos($type, 'enum')) {
            $bind = PDO::PARAM_STR;
        } elseif (preg_match('/(int|double|float|decimal|real|numeric|serial|bit|number)/is', $type)) {
            $bind = PDO::PARAM_INT;
    ...
    public function insert(array $data = [], $replace = false, $getLastInsID = false, $sequence = null)
    {
        ...
        //********************************************************************
        //仅用来连接oracle数据，序列名是 :表明_seq
        $sequence = ltrim($options['table'],$this->prefix).'_seq';
        //********************************************************************
